...
func (r *MemoryRepo) List(limit, skip int) ([]domain.Poll, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	...
	for i := skip; i < len(r.polls) && i < skip+limit; i++ {
...
func (r *MemoryRepo) GetByID(id string) (*domain.Poll, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *MemoryRepo) Create(p domain.Poll) error {
	r.mu.Lock()
	defer r.mu.Unlock()
